...
void SetionMod(mod_type setion_mod)
{
    ...
    unsigned int P_pause_temps=setion_mod.G_PAUSE;
    unsigned int G_pause_temps=setion_mod.P_PAUSE;
...
void GPause(unsigned int G_pause_temps){
    unsigned int gpause_minuteure=G_pause_temps;
    for(int minute=0;minute<G_pause_temps;minute++){
        printf(STYLE_BOLD GREEN "\n        ⏱ Longue pause : %u\n"STYLE_RESET RESET ,gpause_minuteure);
        sleep_sec(60);
        gpause_minuteure--;
    }
}